...
function ProductThumbnail({ imageUrl, name }) {
  ...
  if (!imageUrl || failed) {
    return (
      <div
        className="w-12 h-12 flex-shrink-0 rounded-lg bg-gray-100 flex items-center justify-center text-2xl select-none"
        aria-hidden="true"
      >
        🥫
      </div>
    ...
    <div className="bg-white rounded-xl shadow-sm overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className="flex-shrink-0 bg-emerald-200 text-emerald-800 text-xs font-medium px-2 py-0.5 rounded-full">
            {displayQty}
          </span>
        </div>
        <span className="flex-shrink-0 text-emerald-600 ml-2" aria-hidden="true">
          {open ? '▲' : '▼'}
        </span>
      </button>
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* ── Header ─────────────────────────────────────────────────────── */}
      <header className="sticky top-0 z-10 bg-emerald-700 text-white px-4 py-4 shadow-md">
